...
const CheckLists = () => {
  ...
  const SingleChecklist = ({ listItem, listIndex }) => {
    const {
      card,
      match,
      saveCardChanges,
    ...
    const handleConvertToCard = (item) => {
      ...
      setTimeout(async () => {
        await requestCreateNewCard({ card, listId: sourceId }, id).then(
          (res) => {
  ...
  return card.checklists.map((list, index) => (
    <SingleChecklist key={index} listIndex={index} listItem={list} />
  ));
};
